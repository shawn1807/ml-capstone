...
class QLearningAgent(TrafficLightControl):
    ...
    def setup(self):
        ...
        self.model.add_hidden_layer(len(self.env.roads)*2, activation="linear")
    ...
    def after_signal(self):
        if self.env.t % self.period == 0 and self.learning:
            ...
            #global reward
            g_reward = round((self.env.number_of_car - self.env.stall) * 1./self.env.number_of_car, 2)
            if self.learning_count > 1000 :
                self.learning = False
            next_state = np.array([0 if o is None else 1 for p, o in self.env.roads.values()])
            rewards = []
            for l in self.lights:
                rewards.append(l.ns_rewards + g_reward if l.ns_neuron.output < l.ns_rewards else 0)
                rewards.append(l.ew_rewards + g_reward if l.ew_neuron.output < l.ew_rewards else 0)
                # reset local reward
                l.ns_rewards = 0
                l.ew_rewards = 0
